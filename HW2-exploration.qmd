...
car %>%
  filter(manufacturer != "All" & manufacturer != "Tesla") %>% # exclude All for duplicate and tesla
...
car %>%
  filter(manufacturer != "All", regulatory_class != "All") %>%
  ggplot(aes(x = model_year, y = real_world_mpg, color = regulatory_class)) +
  geom_col() + 
  labs(title = "Real World MPG Over Model Years", x = "Model Year", y = "Real World MPG", color = "Manufacturer") +
  theme_minimal()
